#!/usr/bin/env bash
# Run rf_trainval_scn.py on every dataset, grouping and split.
...
for DATASET in $DATASETS; do
	...
	for GROUPING in $GROUPINGS; do
		...
		for SPLIT in $SPLITS; do
			...
			export RESULTS_DIR=${RESULTS_ROOT}/RF+SCN_${DATASET}_${GROUPING}_${SPLIT}/
			mkdir -p $RESULTS_DIR
...
#			fi
			...
			./rf_trainval_scn.py | tee ${RESULTS_DIR}/rf.log
