#!/usr/bin/env bash
# Run rf_trainval_zooprocess.py on every dataset and grouping.
...
for DATASET in $DATASETS; do
	...
	for GROUPING in $GROUPINGS; do
		...
		export RESULTS_DIR=${RESULTS_ROOT}/RF_${DATASET}_${GROUPING}_`date +%Y-%m-%d-%H-%M`/
		mkdir -p $RESULTS_DIR

		echo Results are written to $RESULTS_DIR

		./rf_trainval_zooprocess.py | tee ${RESULTS_DIR}/random_forest.log
